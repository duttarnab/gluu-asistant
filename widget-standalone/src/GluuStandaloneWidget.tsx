...
import { useStandaloneChat } from './useStandaloneChat';
import { renderMarkdown } from './markdown';
import type { BakedIndex, ChatSource } from './types';

export interface GluuStandaloneWidgetProps {
  /** The baked index produced by `npm run export-index` (import the JSON and pass it). */
  index: BakedIndex;
  /** Ollama base URL. Default: http://localhost:11434 */
  ollamaBaseUrl?: string;
  /** Ollama chat model. Default: llama3 */
  chatModel?: string;
  /** Ollama embed model. Defaults to the model the index was built with. */
  embedModel?: string;
  /** Title shown in the header and welcome screen. */
  title?: string;
  /** Small text under the title. */
  subtitle?: string;
  /** Accent / brand color (any CSS color). */
  accentColor?: string;
...
export function GluuStandaloneWidget({
  index,
  ...
  topK = 5,
  defaultOpen = false,
}: GluuStandaloneWidgetProps) {
  const [open, setOpen] = useState(defaultOpen);
  const [entered, setEntered] = useState(false);
  const [input, setInput] = useState('');

  const { messages, isStreaming, send, clear } = useStandaloneChat({
    index,
    ollamaBaseUrl,
    embedModel,
    chatModel,
